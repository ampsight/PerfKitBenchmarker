...
"""
    For pricing data, please edit the dictionary 'priceData'
...
class Compute(Base):
    ...
    def calculate_confidence(self) -> None:
        ...
        #get the types of threads
        threads = []

        for index, row in self.csv_data.iterrows():
            if row['Thread Count'] not in threads:
                threads.append(row['Thread Count'])
